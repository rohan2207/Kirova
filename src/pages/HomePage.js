...
// List of common McKinney streets for generating addresses
...
const NewHomePage = () => {
  ...
  // Generate a realistic address with a street from McKinney
  const generateAddress = () => {
    const streetNumber = Math.floor(Math.random() * 9000) + 1000;
    const street = MCKINNEY_STREETS[Math.floor(Math.random() * MCKINNEY_STREETS.length)];
    return `${streetNumber} ${street}, McKinney`;
  };

  // Enhanced store data for display with addresses
  const enhancedStores = stores && stores.length ? stores.map(store => {
    return {
      ...
      address: generateAddress() // Add address information
    };
  }) : [];
